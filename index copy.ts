...
export class Query<T> {
   ...
   */
  set(value: T | ((currentValue: T) => T)): T {
    let finalValue = typeof value === "function" ? value(this.get()) : value;
    if (this.path === "") {
  ...
  setItems(
    ...
    if (Array.isArray(updatedRootObject)) {
      // Handle arrays properly
      Object.entries(items).forEach(([key, value]) => {
        const index = parseInt(key, 10);
        if (!isNaN(index)) {
          updatedRootObject[index] = value;
        }
      });
    } else {
      // For objects, Object.assign works fine
      Object.assign(updatedRootObject, items);
    }
  ...
  qfind<U = T extends (infer U)[] ? U : never>( // Introduce new generic 'U'
    ...
    //@ts-ignore
    return new Query<U>(this.path + "." + index, this.rootObj);
  }
  qEach<U = T extends (infer U)[] ? U : never>(
    fn: (xQuery: Query<U>) => void
  ): this {
  ...
  qFilter<U = T extends (infer U)[] ? U : never>(
    ...
    transformFn: (xQuery: Query<U>) => void
  ): this {
  ...
  setItem<K extends keyof T>(
    ...
    const finalValue =
      typeof value === "function" ? value(this.get()?.[keyOrIndex]) : value;
    if (Array.isArray(this.get())) {
      //@ts-ignore
      const updatedArray = [...this.get()];
      updatedArray[keyOrIndex] = finalValue;
      //@ts-ignore
